...
const reversedWordsOf = function (strings) {
  return [...strings].map((sentence) =>
    sentence.split(" ").map(reverseString).join(" ")
  );
};
...
const calculateAreas = function (rectangles) {
  return rectangles.map((dimensions) => dimensions.height * dimensions.width);
};
...
const fullNames = function (objects) {
  return objects.map((person) => person.firstName + " " + person.lastName);
};
...
const addPunctuation = function (messages, punctuations) {
  return messages.flatMap((message) =>
    punctuations.map((punctuation) => message + punctuation)
  );
};
