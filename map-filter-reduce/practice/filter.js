...
const filterIncompleteProfiles = function (users) {
  return users.filter((product) => product.profileComplete);
};
...
const filterRecentOrders = function (orders) {
  return orders.filter(isIn);
};

// products with a price lower than the average [{name: "item1", price: 10}, {name: "item2", price: 20}, {name: "item3", price: 5}] => [{name: "item1", price: 10}, {name: "item3", price: 5}]
const average = function (array) {
  return array.reduce((num1, num2) => num1 + num2, 0) / array.length;
};

const filterBelowAveragePrice = function (products) {
  const avg = average(products.map((product) => product.price));
...
const filterBirthdaysThisMonth = function (people) {
  return people.filter(({ birthDate }) => birthDate.split("-")[1] === "12");
};
...
const averageOf = (sum, value) => sum + value;
...
const filterTopRatedBooks = function (books) {
  const avg = books.reduce((sum, book) => sum + book.rating, 0) / books.length;
...
const filterCarsByFeatureType = (cars, featureType) => {};
...
const filterTeamsBySkill = (teams, skill) => {};
...
const filterStudentsByGrade = (students, minGrade) => {};
...
const filterBooksByAward = (books, award) => {};
